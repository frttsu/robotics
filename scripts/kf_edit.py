...
class KalmanFilter: ###kf4init
    ...
    def observation_update(self, observation):  #追加
        for d in observation:
            ...
            K = self.belief_cov.dot(H.T).dot(np.linalg.inv(Q + H.dot(self.belief_cov).dot(H.T)))
            self.belief.mean += K.dot(z - estimated_z)
            self.belief_cov = (np.eye(3) - K.dot(H)).dot(self.belief_cov)
            self.pose = self.belief.mean
    ...
    def draw(self, ax, elems):
        ###xy平面上の誤差の3シグマ範囲###
        e = sigma_ellipse(self.belief.mean[0:2], self.belief_cov[0:2, 0:2], 3)
